...
def get_art_abs(story_file):
    ...
    extract_sents, extract_ids, fs, ps, rs, max_Rouge_l_r = get_extract_summary(story_file, article_sents, highlights)
...
def get_extract_summary(story_file, article_sents, abstract_sents):
    ...
    if len(extract_sents) != 0 and len(abstract_sents) != 0:
    ...
    #  print '%s %s %s %s %s %s' % (len(extract_sents), len(extract_ids), len(fscores), len(precisions), len(recalls), max_Rouge_l_r)

    return extract_sents, extract_ids, fscores, precisions, recalls, max_Rouge_l_r


def write_to_bin(dir_loc, out_file, makevocab=False):
    """Reads the tokenized .story files corresponding to the urls listed in the url_file and writes them to a out_file."""
    print "Making bin file for files listed in %s" % dir_loc
    story_fnames = os.listdir(dir_loc)
    print "Detected %s files." % len(story_fnames)
    ...
    with open(out_file, 'wb') as writer:
        for idx, s in tqdm(enumerate(story_fnames)):
            ...
            if os.path.isfile(os.path.join(dir_loc, s)):
                story_file = os.path.join(dir_loc, s)
            else:
            ...
            if len(article_sents) == 0 or len(extract_sents) == 0:
                print 'Skipping story %s' % story_file
                continue
...
if __name__ == '__main__':
    ...
    write_to_bin('/home/sl1/S3727321/unified-summarization/data/gigawordProcessed/test',
                 os.path.join(finished_files_dir, "test.bin"))
    write_to_bin('/home/sl1/S3727321/unified-summarization/data/gigawordProcessed/val',
                 os.path.join(finished_files_dir, "val.bin"))
    write_to_bin('/home/sl1/S3727321/unified-summarization/data/gigawordProcessed/train',
                 os.path.join(finished_files_dir, "train.bin"), makevocab=True)
